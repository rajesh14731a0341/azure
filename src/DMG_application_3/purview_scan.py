...
ENTRY POINT — run this script directly:
  python purview_scan.py

  The script runs a dependency pre-flight check on startup (installing
  any missing packages automatically) before starting the scan.
  You do NOT need to run check_and_install.py separately.
...
# ═══════════════════════════════════════════════════════════════════════
#  DEPENDENCY PRE-FLIGHT  (runs before any other imports)
#  Mirrors check_and_install.py — no need to run it separately.
# ═══════════════════════════════════════════════════════════════════════

def _preflight():
    """Check required packages; install missing ones; abort on fatal failure."""
    ...
    PKGS = [
        ...
        ("pyodbc",             "pyodbc",             False),
        ("pymssql",            "pymssql",            False),
        ("azure.storage.blob", "azure-storage-blob", False),
    ]

    print()
    print(HDR("=" * 68))
    print(HDR("  purview_scan.py — Dependency Pre-flight Check"))
    print(HDR("=" * 68))

    results = {}
    to_install = []
    for imp, pip, req in PKGS:
        ok = True
        try:
        ...
        results[imp] = ok
        if not ok:
            to_install.append((imp, pip, req))
        status = OK("✓ installed") if ok else WARN("✗ missing — installing...")
        print(f"  {imp:<26} {status}")

    if to_install:
        print()
        for imp, pip, req in to_install:
            ...
            if r.returncode == 0:
                ...
                try:
                    ...
                    print(f"  {WARN('installed but import still failed')}")
            else:
                print(f"  {ERR('FAILED')}  (run: pip install {pip})")

    core_ok = all(results.get(imp, False) for imp, _, req in PKGS if req)
    db_ok   = results.get("pyodbc", False) or results.get("pymssql", False)

    print()
    if core_ok and db_ok:
        print(OK("  ✓ Environment ready."))
    elif core_ok:
        print(WARN("  ⚠ No SQL driver found — DB push will be skipped.  "
                   "Install pymssql for DB support."))
    else:
        missing = [imp for imp, _, req in PKGS if req and not results.get(imp, False)]
        print(ERR(f"  ✗ Required package(s) missing: {', '.join(missing)}"))
...
# Now import the packages that were just guaranteed to be present
import requests

# ── Compliance module (optional — skipped gracefully if not present) ──
try:
    from purview_compliance import run_compliance_check as _run_compliance
...
# ── Compliance check settings (optional) ─────────────────────────────
# Set YAML_SOURCE to a GitHub URL or local path to run the compliance
# check automatically after every scan.  Leave blank to skip.
YAML_SOURCE        = _get("YAML_SOURCE",         "")
COMPLIANCE_DMG     = _get("COMPLIANCE_DMG",       "")   # comma-sep DMG IDs
COMPLIANCE_REPORT  = _get("COMPLIANCE_REPORT",    "compliance_report.xlsx")

# ── Compliance DB load settings (optional) ────────────────────────────
# COMPLIANCE_DB_LOAD = true   → push compliance rows to Azure SQL after Excel
# COMPLIANCE_DB_LOAD = false  → skip DB push (default)
# When true, all three table names + schema are MANDATORY in config.
# Script will fail immediately if any are blank.
COMPLIANCE_DB_LOAD          = _get("COMPLIANCE_DB_LOAD",          "false").lower() == "true"
COMPLIANCE_DB_SCHEMA        = _get("COMPLIANCE_DB_SCHEMA",        "").strip()
COMPLIANCE_DB_TABLE         = _get("COMPLIANCE_DB_TABLE",         "").strip()
COMPLIANCE_DB_HISTORY_TABLE = _get("COMPLIANCE_DB_HISTORY_TABLE", "").strip()
COMPLIANCE_DB_SUMMARY_TABLE = _get("COMPLIANCE_DB_SUMMARY_TABLE", "").strip()
COMPLIANCE_DB_COL_HISTORY_TABLE = _get("COMPLIANCE_DB_COL_HISTORY_TABLE", "compliance_column_history_details").strip()
# SQL connection — reuse from the main SQL settings
COMPLIANCE_SQL_SERVER   = _get("SQL_SERVER",   "")
COMPLIANCE_SQL_DATABASE = _get("SQL_DATABASE", "")
COMPLIANCE_SQL_USER     = _get("SQL_USER",     "")
COMPLIANCE_SQL_PASSWORD = (_get("SQL_PASSWORD", "")
                            or os.environ.get("SQL_PASSWORD", ""))

# ── YAML source mode flags ────────────────────────────────────────────
# Priority when more than one is true: GitHub > Azure Blob > Local.
YAML_SOURCE_GITHUB     = _get("YAML_SOURCE_GITHUB",     "false").lower() == "true"
YAML_SOURCE_AZURE_BLOB = _get("YAML_SOURCE_AZURE_BLOB", "false").lower() == "true"
YAML_SOURCE_LOCAL      = _get("YAML_SOURCE_LOCAL",      "false").lower() == "true"
# GitHub auth — only needed for private repos
YAML_GITHUB_TOKEN      = (_get("YAML_GITHUB_TOKEN", "")
                           or os.environ.get("YAML_GITHUB_TOKEN", ""))
# Azure Blob params — only needed when YAML_SOURCE_AZURE_BLOB = true
YAML_BLOB_ACCOUNT      = _get("YAML_BLOB_ACCOUNT",   "")
YAML_BLOB_CONTAINER    = _get("YAML_BLOB_CONTAINER",  "")
YAML_BLOB_PREFIX       = _get("YAML_BLOB_PREFIX",     "")
YAML_BLOB_SAS_TOKEN    = (_get("YAML_BLOB_SAS_TOKEN", "")
                           or os.environ.get("YAML_BLOB_SAS_TOKEN", ""))
YAML_BLOB_CONN_STR     = (_get("YAML_BLOB_CONN_STR",  "")
                           or os.environ.get("YAML_BLOB_CONN_STR", ""))

# Validate Azure Blob settings early when that mode is active
if YAML_SOURCE_AZURE_BLOB:
    _missing_blob = [k for k, v in [
        ("YAML_BLOB_ACCOUNT",    YAML_BLOB_ACCOUNT),
        ("YAML_BLOB_CONTAINER",  YAML_BLOB_CONTAINER),
    ] if not v]
    if _missing_blob:
        print(f"\n[ERROR] YAML_SOURCE_AZURE_BLOB = true but these config keys are "
              f"missing or blank in '{_config_path}':")
        for k in _missing_blob:
            print(f"  * {k}")
        print("\nYAML_BLOB_ACCOUNT and YAML_BLOB_CONTAINER are mandatory when "
              "YAML_SOURCE_AZURE_BLOB = true.\n")
        import sys as _sys; _sys.exit(1)

# Validate DB settings early so the script fails fast before any API calls
if COMPLIANCE_DB_LOAD:
    _missing_db = [k for k, v in [
        ("COMPLIANCE_DB_SCHEMA",        COMPLIANCE_DB_SCHEMA),
        ("COMPLIANCE_DB_TABLE",         COMPLIANCE_DB_TABLE),
        ("COMPLIANCE_DB_HISTORY_TABLE", COMPLIANCE_DB_HISTORY_TABLE),
        ("COMPLIANCE_DB_SUMMARY_TABLE", COMPLIANCE_DB_SUMMARY_TABLE),
        ("COMPLIANCE_DB_COL_HISTORY_TABLE", COMPLIANCE_DB_COL_HISTORY_TABLE),
        ("SQL_SERVER",                  COMPLIANCE_SQL_SERVER),
        ("SQL_DATABASE",                COMPLIANCE_SQL_DATABASE),
        ("SQL_USER",                    COMPLIANCE_SQL_USER),
        ("SQL_PASSWORD",                COMPLIANCE_SQL_PASSWORD),
    ] if not v]
    if _missing_db:
        print(f"\n[ERROR] COMPLIANCE_DB_LOAD = true but these config keys are "
              f"missing or blank in '{_config_path}':")
        for k in _missing_db:
            print(f"  * {k}")
        print("\nAll eight keys are mandatory when COMPLIANCE_DB_LOAD = true.\n")
        import sys as _sys; _sys.exit(1)
...
def fetch_assets(collection_ids: list = None):
    """
    Fetch all leaf assets via the Search API.

    When collection_ids is provided (a list of Purview collection name/IDs),
    the filter is pushed INTO the Search API request body so only assets
    belonging to those collections are returned.  This is far faster than
    fetching everything and discarding non-matching assets afterward.

    When collection_ids is None / empty, all assets are fetched (full scan).
    """
    if collection_ids:
        log_section(f"STEP 3 — Fetch assets (search, filtered to "
                    f"{len(collection_ids)} collection(s))")
        log(f"  Collection IDs in filter: {collection_ids}")
    else:
        log_section("STEP 3 — Fetch all assets (search, no collection filter)")

    items, page, tk = [], 1, None
    while True:
        body: dict = {"keywords": "*", "limit": SEARCH_PAGE_SIZE}
        if tk:
            body["continuationToken"] = tk
        # Push collection filter into the API request body so Purview
        # returns only matching assets — avoids fetching and discarding
        # the entire catalog when only a subset of collections is needed.
        if collection_ids:
            body["filter"] = {
                "or": [
                    {"collectionId": cid}
...
def write_json(path, data, label):
    try:
        ...
        print(f"[OK]   {label}: {Path(path).name} ({size_mb:.2f} MB) [{len(data)} records]")
...
def main():
    ...
    # ── Resolve collection filter → collection IDs for Search API ─────
    # coll_paths is { collection_name_id: "Friendly / Hierarchy / Path" }
    # We need to hand the *internal name/ID* (the key) to the Search API.
    # When COLLECTIONS_FILTER is set, find every collection whose resolved
    # hierarchy path starts with any of the filter strings.
    search_collection_ids = None   # None = no filter (full scan)
    if COLLECTIONS_FILTER:
        search_collection_ids = [
            cid
            for cid, cpath in coll_paths.items()
            if cpath and coll_matches(cpath)
        ]
        if search_collection_ids:
            log(f"Collection filter resolved to {len(search_collection_ids)} "
                f"collection ID(s) — pushing into Search API body "
                f"(skips fetching entire catalog)", "OK")
            for cid in search_collection_ids:
                log(f"  → {cid}  ({coll_paths[cid]})")
        else:
            log("Collection filter active but NO matching collection IDs found "
                "in the catalog.  Proceeding with full scan (filter will still "
                "be applied per-asset during snapshot build).", "WARN")
            search_collection_ids = None
    ...
    # ── STEP 3: Assets (search, collection-filtered at API level) ────
    all_assets  = fetch_assets(collection_ids=search_collection_ids)
    ...
    log(f"  Written: {snap_path.name}  "
        ...
        f"{enriched_assets_in_snap} assets,  {size_mb:.2f} MB)", "OK")
...
""", flush=True)
    ...
    # ── STEP 7: Compliance check (runs only if YAML_SOURCE is configured) ──
    if YAML_SOURCE:
        if not _HAS_COMPLIANCE:
            log("purview_compliance.py not found — skipping compliance check. "
                "Place purview_compliance.py in the same folder as this script.",
                "WARN")
        else:
            dmg_filter = [d.strip() for d in COMPLIANCE_DMG.split(",")
                          if d.strip()]

            # Excel goes next to the running script (not inside json_output/).
            # Overwritten completely on every run — never accumulates.
            report_path = str(Path(__file__).parent / COMPLIANCE_REPORT)

            comp_result = _run_compliance(
                ...
                # GitHub auth
                github_token           = YAML_GITHUB_TOKEN,
                # Azure Blob params
                blob_account           = YAML_BLOB_ACCOUNT,
                blob_container         = YAML_BLOB_CONTAINER,
                blob_prefix            = YAML_BLOB_PREFIX,
                blob_sas_token         = YAML_BLOB_SAS_TOKEN,
                blob_conn_str          = YAML_BLOB_CONN_STR,
                # DB push params — all resolved from config, passed through
                db_load              = COMPLIANCE_DB_LOAD,
                db_schema            = COMPLIANCE_DB_SCHEMA,
                db_table             = COMPLIANCE_DB_TABLE,
                db_history_table     = COMPLIANCE_DB_HISTORY_TABLE,
                db_summary_table     = COMPLIANCE_DB_SUMMARY_TABLE,
                db_col_history_table = COMPLIANCE_DB_COL_HISTORY_TABLE,
                db_server       = COMPLIANCE_SQL_SERVER,
                db_database     = COMPLIANCE_SQL_DATABASE,
                db_user         = COMPLIANCE_SQL_USER,
                db_password     = COMPLIANCE_SQL_PASSWORD,
            )

            # ── Log results from compliance check ──────────────────────
            log(f"Compliance report : {report_path}", "OK")
            log(f"  Total columns   : {comp_result.get('total', 0)}", "INFO")
            log(f"  Compliant       : {comp_result.get('compliant', 0)}", "OK")
            log(f"  Non-compliant   : {comp_result.get('non_compliant', 0)}",
                "WARN" if comp_result.get('non_compliant', 0) else "INFO")
            log(f"  Not in Purview  : {comp_result.get('not_found', 0)}",
                "WARN" if comp_result.get('not_found', 0) else "INFO")

            if COMPLIANCE_DB_LOAD:
                db_status = comp_result.get("db_status", "unknown")
                db_rows   = comp_result.get("db_rows_loaded", 0)
                if db_status == "success":
                    log(f"  DB push         : SUCCESS — {db_rows} rows → "
                        f"[{COMPLIANCE_DB_SCHEMA}].[{COMPLIANCE_DB_TABLE}]  "
                        f"| [{COMPLIANCE_DB_SCHEMA}].[{COMPLIANCE_DB_HISTORY_TABLE}]  "
                        f"| [{COMPLIANCE_DB_SCHEMA}].[{COMPLIANCE_DB_SUMMARY_TABLE}]  "
                        f"| [{COMPLIANCE_DB_SCHEMA}].[{COMPLIANCE_DB_COL_HISTORY_TABLE}]", "OK")
                elif db_status == "skipped":
                    log(f"  DB push         : SKIPPED (0 rows produced)", "INFO")
                else:
                    log(f"  DB push         : FAILED — {comp_result.get('db_error','')}",
                        "ERROR")
    else:
        log("YAML_SOURCE not set in config — compliance check skipped. "
            "Set YAML_SOURCE in purview_config.ini to enable.", "INFO")
