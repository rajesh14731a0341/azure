...
def _check_column(yaml_col: dict, snap_records: list) -> dict:
    ...
    return {
        ...
        "reasons":                reasons,
        # individual snap records for the column-history-details table
        ...
        for dp in (doc.get("data_products") or []):
            ...
            for idx, col in enumerate(columns, start=1):
                ...
                all_rows.append({
                    ...
                    # individual snap records — used to build column-level history detail rows
                    "snap_records":           result.get("snap_records", []),
...
def _build_excel(rows: list, out_path: str, run_ts: str):
    ...
    # Group-label row
    # Columns: 1-7 YAML | 8-11 Purview Snapshot | 12 col | 13 YAML Version | 14-15 Result
    groups = [
        (1,  7,  "FROM YAML  (Master / Reference)",               _C_BLUE),
        (8,  11, "FROM PURVIEW SNAPSHOT  (Classification-based)", "FF4472C4"),
        (12, 12, "COLLECTION PATH",                               "FF538135"),
        (13, 13, "YAML VERSION",                                  "FF70AD47"),
        (14, 15, "COMPLIANCE RESULT",                             _C_PURPLE),
    ]
    ...
    # Column headers  (15 columns total — yaml_column_name removed)
    hdrs = [
        # YAML (1-7)
        ("DMG Name",                        _C_BLUE),
    ...
    for ri, row in enumerate(rows, start=4):
        ...
        for ci, val in enumerate(vals, start=1):
            ...
            fg = comp_fg if is_comp else (_C_WHITE if count_zero else (
                _C_RED if cls_not_found and ci == 8 else _C_BLACK))
            _cell(ws, ri, ci, val,
                  bg=bg, fg=fg, bold=(is_comp or count_zero),
                  align="center" if ci in (1, 3, 7, 10, 11, 13, 14) else "left",
                  wrap=ci in (12, 15))
        ws.row_dimensions[ri].height = 18

    # Column widths  (15 cols — DMG, App, Region, PurviewCls, SensLabel, Status, Date,
    #                            PurviewCls(M), PurviewSens, ColCnt, AssetCnt, CollPath, Ver, Comp, Reason)
    widths = [14, 26, 10, 36, 24, 10, 14,
...
#    • Lookup key: (dmg_name, app_name, yaml_purview_cls)  — deduplicated in Python.
#    • No unique constraint; row_id (UUID) is the only PK.
#    • First scan for a record  → INSERT.
#    • Re-scan with no changes  → only last_scanned_at is updated (silent).
#    • Re-scan with any change  → UPDATE the main row (new values, by row_id).
#
...
# ── DDL: main table ────────────────────────────────────────────────────
# row_id is the only PK — NO unique constraints, NO FK constraints.
# All tables are fully independent; no mapping between them.
# Deduplication is handled in Python via the existing-rows dict keyed
# by (dmg_name, app_name, yaml_purview_cls) — duplicates within a single
# run are collapsed before touching the DB so no constraint is needed.
_DDL_TABLE = """\
...
# Append-only — INSERT on EVERY run for EVERY record.
# No unique constraints, no FK constraints. Fully independent table.
_DDL_HISTORY_TABLE = """\
...
# ── DDL: summary table ─────────────────────────────────────────────────
# Append-only run-level summary — INSERT on every run, never updated.
# One row per DMG / application per run. No unique constraints, no FK.
_DDL_SUMMARY_TABLE = """\
...
# ── DDL: column-level history details table ────────────────────────────
# Append-only.  One row per individual Purview column per run.
# Unlike the aggregate history table (which stores counts), this table
# stores one row per actual Purview column — giving the exact column name,
# fully-qualified asset name, table name, and collection path.
_DDL_COL_HISTORY_TABLE = """\
IF OBJECT_ID(N'[{schema}].[{col_history_table}]', N'U') IS NULL
BEGIN
CREATE TABLE [{schema}].[{col_history_table}] (
    history_id                  NVARCHAR(36)    NOT NULL DEFAULT NEWID(),
    -- Compliance context
    dmg_name                    NVARCHAR(200)   NOT NULL,
    app_name                    NVARCHAR(500)   NOT NULL,
    ...
    yaml_version                NVARCHAR(100)   NULL,
    yaml_effective_date         NVARCHAR(100)   NULL,
    -- FROM PURVIEW SNAPSHOT (individual column-level, not aggregated)
    purview_classification      NVARCHAR(500)   NULL,
    purview_sensitivity         NVARCHAR(2000)  NULL,
    purview_column_name         NVARCHAR(500)   NULL,
    purview_asset_fqn           NVARCHAR(2000)  NULL,   -- fully qualified name (db.schema.table)
    purview_table               NVARCHAR(500)   NULL,   -- table name only
    purview_collection_path     NVARCHAR(2000)  NULL,
    -- COMPLIANCE RESULT
    compliant                   NVARCHAR(3)     NULL,
    reason                      NVARCHAR(MAX)   NULL,
    CONSTRAINT PK_{col_history_table} PRIMARY KEY (history_id)
);
PRINT '{col_history_table} created.';
END
ELSE
    PRINT '{col_history_table} already exists.';"""
...
_MAIN_COL_IDX = {
    ...
    "region":                  6,
    "yaml_purview_cls":        7,
    "yaml_sensitivity_label":  8,
    "yaml_status":             9,
    "yaml_version":            10,
    "yaml_effective_date":     11,
    "purview_classification":  12,
    "purview_sensitivity":     13,
    "purview_column_count":    14,
    "purview_asset_count":     15,
    "collection_path":         16,
    "compliant":               17,
    "reason":                  18,
}
...
def push_to_db(rows:                  list,
               ...
               db_col_history_table:  str = "") -> tuple:
    """
    ...
    ──────────────────────────────────────────
    Lookup key = (dmg_name, app_name, yaml_purview_cls)
    Multiple YAML columns may share the same purview_classification, so
    within a single run duplicates are collapsed (last-wins) before any DB
    write.  The table has NO unique constraint — row_id (UUID) is the only PK.
    1. NOT FOUND  → INSERT into detail table.
    2. FOUND, no change  → UPDATE last_scanned_at only (silent).
    3. FOUND, something changed  → UPDATE the detail row (by row_id).
    ...
    col_history_table = db_col_history_table or "compliance_column_history_details"
    ...
    _log(f"  Col-history table : [{db_schema}].[{col_history_table}]")
    ...
    try:
        ...
        # ── 2b. Drop legacy UQ constraint if it still exists ──────────
        # Earlier versions created UQ_{table}_key which causes duplicate-key
        # errors when multiple YAML columns share the same classification.
        # Safe to drop: we deduplicate in Python before any DB write.
        cur.execute(f"""
            IF EXISTS (
                SELECT 1 FROM sys.key_constraints
                WHERE name = N'UQ_{db_table}_key'
                  AND parent_object_id = OBJECT_ID(N'[{db_schema}].[{db_table}]')
            )
            BEGIN
                ALTER TABLE [{db_schema}].[{db_table}]
                    DROP CONSTRAINT [UQ_{db_table}_key];
                PRINT 'Dropped legacy UQ_{db_table}_key constraint.';
            END
        """)
        ...
        # ── 5a. Ensure column-history-details table exists ─────────────
        for stmt in _DDL_COL_HISTORY_TABLE.format(
                schema=db_schema, col_history_table=col_history_table).split("\nGO\n"):
            stmt = stmt.strip()
            if stmt:
                cur.execute(stmt)

        _log(f"  Schema and all four tables ready", "OK")

        # ── 5b. Fetch all existing main-table rows for the incoming DMGs ─
        # Pull only the DMGs present in this scan — avoids loading the whole table.
        # Lookup key: (dmg_name, app_name, yaml_purview_cls)
        # Because the table has NO unique constraint on this triple, multiple DB
        # rows for the same key are possible from past runs.  We keep only the
        # most-recently-scanned one (last wins) for the update/touch comparison.
        incoming_dmgs = list({r["dmg_name"] for r in rows if r.get("dmg_name")})
        existing: dict = {}   # (dmg_name, app_name, yaml_purview_cls) → db_row
        if incoming_dmgs:
            placeholders = ",".join([ph] * len(incoming_dmgs))
            cur.execute(
                f"SELECT * FROM [{db_schema}].[{db_table}] "
                f"WHERE dmg_name IN ({placeholders}) "
                f"ORDER BY last_scanned_at DESC",
                incoming_dmgs,
            )
            for db_row in cur.fetchall():
                key = (
                    ...
                    _val_str(db_row[_MAIN_COL_IDX["yaml_purview_cls"]]),
                )
                # Keep only first occurrence (most recent due to ORDER BY)
                if key not in existing:
                    existing[key] = db_row
        _log(f"  Existing records fetched for comparison: {len(existing)}")

        # ── 6. Deduplicate incoming rows then categorise for main table ─
        # Multiple YAML columns can share the same purview_classification.
        # De-dupe within this run: last row for each (dmg, app, cls) key wins.
        deduped: dict = {}   # (dmg_name, app_name, yaml_purview_cls) → row
        for row in rows:
            key = (
                _val_str(row.get("dmg_name")),
                _val_str(row.get("app_name")),
                _val_str(row.get("yaml_purview_cls")),
            )
            deduped[key] = row   # last-wins within this run
        _log(f"  Incoming rows          : {len(rows)}")
        _log(f"  After dedup (main tbl) : {len(deduped)}  "
             f"({len(rows) - len(deduped)} duplicate key(s) collapsed)")
        ...
        if to_update:
            ...
            for r, db_row, changed_cols in to_update:
                cur.execute(upd_sql, (
                    ...
                    r["reason"],
                    db_row[_MAIN_COL_IDX["row_id"]],
                ))
                _log(f"    UPDATED  [{r['dmg_name']}] "
                     f"{r['yaml_purview_cls']}  — changed: {', '.join(changed_cols)}")
        ...
        # ── 12. ALWAYS-INSERT per-Purview-column rows into col-history-details ──
        # One row per individual Purview column record (not aggregated by count).
        # purview_column_name, purview_asset_fqn, purview_table, purview_collection_path
        # come directly from each snap_record attached to the result row.
        _phs_col = ", ".join([ph] * 18)
        col_hist_sql = f"""
            INSERT INTO [{db_schema}].[{col_history_table}] (
                dmg_name, app_name, region,
                yaml_purview_cls,
                yaml_sensitivity_label, yaml_status,
                yaml_version, yaml_effective_date,
        ...
        col_hist_params = []
        for r in rows:
            snap_recs = r.get("snap_records") or []
            if snap_recs:
                for sr in snap_recs:
                    col_hist_params.append((
                        r["dmg_name"],          r["app_name"],
                        r.get("region", ""),
                        r["yaml_purview_cls"],  r["yaml_sensitivity_label"],
                        r["yaml_status"],       r["yaml_version"],
                        r["yaml_effective_date"],
                        r["purview_classification"],
                        r["purview_sensitivity"],
                        (sr.get("column_name") or ""),
                        (sr.get("asset_qualified_name") or ""),
                        (sr.get("asset_name") or ""),
                        (sr.get("collection_hierarchy_path") or ""),
                        r["compliant"],         r["reason"],
                        datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),
                        str(_uuid2.uuid4()),
                    ))
            else:
                # No matching Purview columns — still record one row with blank purview fields
                col_hist_params.append((
                    r["dmg_name"],          r["app_name"],
                    r.get("region", ""),
                    r["yaml_purview_cls"],  r["yaml_sensitivity_label"],
        ...
        if col_hist_params:
            cur.executemany(col_hist_sql, col_hist_params)
            _log(f"  Inserted {len(col_hist_params)} row(s) into column-history-details table", "OK")

        conn.commit()

        total_processed = len(to_insert) + len(to_update) + len(to_touch)
        _col_hist_count = len(col_hist_params) if 'col_hist_params' in dir() else 0
        _log(f"  DB push complete: {total_processed} main-table row(s) processed  "
             ...
             f"| {len(rows)} history rows  | {len(_sum_agg)} summary rows"
             f"| {_col_hist_count} col-history-detail rows", "OK")
...
def run_compliance_check(snapshot_path: str,
    ...
    if db_load:
        ...
        _log(f"  Target (col-history) : [{db_schema}].[{db_col_history_table}]  (INSERT every run)")
        _log(f"  Rows to process  : {len(rows)}")
        _log(f"  Mode: upsert detail / always-insert history + summary + col-history")
        db_rows_done, db_error_msg = push_to_db(
            ...
            db_col_history_table = db_col_history_table,
        )
        if db_error_msg:
