...
# ═══════════════════════════════════════════════════════════════════════
#  DB PUSH  — upsert into main table + append-only audit history table
#
...
#    • Always holds the current live state of every compliance record.
...
#    • Re-scan with any change  → UPDATE the main row; the OLD values are
#                                  written to the history table first.
#
#  HISTORY TABLE  ({schema}.{table}_history)
#    • Append-only audit log — rows are never updated or deleted.
#    • A row is written ONLY when a change is detected — not on every scan.
#    • Each row captures the PREVIOUS state of the record, together with
#      recorded_at (the moment the old values were replaced) so that any
#      future query can answer "what was column X before, and what is it now?".
#    • scan_run_id is intentionally excluded from both tables — it changes
#      every run and carries no business meaning for change detection.
#
#  RETURNS: (rows_loaded: int, error_message: str | None)
# ═══════════════════════════════════════════════════════════════════════

# ── DDL: shared schema guard ───────────────────────────────────────────
_DDL_SCHEMA = """\
...
# Append-only.  Each row = the PREVIOUS state of a main-table record,
# written at the moment a change was detected.
# Query pattern:
#   SELECT * FROM history WHERE dmg_name=? AND yaml_column_name=?
#   ORDER BY recorded_at DESC
# → row 0 = most-recent previous state; main table = current state.
_DDL_HISTORY_TABLE = """\
...
CREATE TABLE [{schema}].[{history_table}] (
    ...
    main_row_id              NVARCHAR(36)    NOT NULL,
    -- Denormalised natural key (no join needed for audit queries)
    dmg_name                 NVARCHAR(200)   NOT NULL,
    app_name                 NVARCHAR(500)   NOT NULL,
    yaml_column_name         NVARCHAR(500)   NOT NULL,
    -- When was this old state replaced?
    recorded_at              DATETIME2       NOT NULL DEFAULT GETUTCDATE(),
    -- Snapshot of EVERY business column as it was BEFORE the change
    pipeda_classification    NVARCHAR(500)   NULL,
    yaml_purview_cls         NVARCHAR(500)   NULL,
    yaml_sensitivity_label   NVARCHAR(500)   NULL,
    yaml_status              NVARCHAR(100)   NULL,
...
# ── Business columns used for change detection ─────────────────────────
# scan_run_id / row_id / timestamps are deliberately excluded — they
# change every run and carry no business meaning.
_COMPARE_COLS = [
    "pipeda_classification",
    "yaml_purview_cls",
...
def push_to_db(rows:        list,
               scan_run_id: str,       # kept for signature compat; not stored
               db_server:   str,
               db_database: str,
               db_user:     str,
               db_password: str,
               ...
               db_table:    str) -> tuple:
    """
    Upsert compliance rows into the main table and write changed rows to the
    history (audit) table.
    ...
    1. Look up the key in the main table.
    2. NOT FOUND  → INSERT into main table.  No history entry (first appearance).
    3. FOUND, no change  → UPDATE last_scanned_at only (silent, no history entry).
    4. FOUND, something changed  → INSERT the OLD values into the history table
                                    (records the previous state with recorded_at
                                    timestamp), then UPDATE the main table row
                                    with the new values and set last_updated_at.

    scan_run_id is accepted for backward compatibility but is never written
    to the database — it changes every run and is meaningless for change detection.
    ...
    history_table = f"{db_table}_history"
    ...
    _log(f"  Main  table       : [{db_schema}].[{db_table}]")
    ...
    try:
        ...
        _log(f"  Schema and both tables ready", "OK")

        # ── 4. Fetch all existing main-table rows for the incoming DMGs ─
        ...
        for row in rows:
            ...
            if key not in existing:
                ...
                if changed:
                    ...
                    hist_insert.append(db_row)   # snapshot the OLD state
                else:
        ...
        if to_insert:
            ...
            ins_sql = f"""
                INSERT INTO [{db_schema}].[{db_table}] (
                    dmg_name, app_name, yaml_column_name,
                    pipeda_classification, yaml_purview_cls,
            ...
            cur.executemany(ins_sql, [
                (
                    ...
                    r["yaml_column_name"],  r["pipeda_classification"],
        ...
        # ── 7. Write OLD state to history table before updating main ───
        if hist_insert:
            _phs17 = ", ".join([ph] * 17)
            hist_sql = f"""
            ...
            cur.executemany(hist_sql, [
                (
                    db_row[I["row_id"]],
                    db_row[I["dmg_name"]],          db_row[I["app_name"]],
                    db_row[I["yaml_column_name"]],  db_row[I["pipeda_classification"]],
                    db_row[I["yaml_purview_cls"]],  db_row[I["yaml_sensitivity_label"]],
                    db_row[I["yaml_status"]],        db_row[I["yaml_version"]],
                    db_row[I["yaml_effective_date"]],
                    db_row[I["purview_classification"]], db_row[I["purview_sensitivity"]],
                    db_row[I["purview_column_count"]],   db_row[I["purview_asset_count"]],
                    db_row[I["collection_path"]],   db_row[I["compliant"]],
                    db_row[I["reason"]],
                )
                for db_row in hist_insert
            ])
            _log(f"  Inserted {len(hist_insert)} old-state snapshot(s) into history table", "OK")
        ...
        if to_update:
            ...
            for r, db_row, changed_cols in to_update:
                cur.execute(upd_sql, (
                    r["pipeda_classification"],  r["yaml_purview_cls"],
                    r["yaml_sensitivity_label"], r["yaml_status"],
                    r["yaml_version"],           r["yaml_effective_date"],
                    r["purview_classification"], r["purview_sensitivity"],
                    r["purview_column_count"],   r["purview_asset_count"],
        ...
        _log(f"  DB push complete: {total_processed} row(s) processed  "
...
def run_compliance_check(snapshot_path: str,
    ...
    if db_load:
        ...
        _log(f"  Target (main)    : [{db_schema}].[{db_table}]  on  {db_server}/{db_database}")
        _log(f"  Target (history) : [{db_schema}].[{db_table}_history]")
        _log(f"  Rows to process  : {len(rows)}")
        _log(f"  Mode: upsert — INSERT new / UPDATE changed / history for all changes")
