...
# When true, COMPLIANCE_DB_SCHEMA and COMPLIANCE_DB_TABLE are MANDATORY.
# Script will fail immediately if either is blank.
...
# SQL connection — reuse or override from the main SQL settings
...
if COMPLIANCE_DB_LOAD:
    ...
    if _missing_db:
        ...
        for k in _missing_db:
            print(f"  * {k}")
        print("\nAll six keys are mandatory when COMPLIANCE_DB_LOAD = true.\n")
        import sys as _sys; _sys.exit(1)
    ...
    if YAML_SOURCE:
        if not _HAS_COMPLIANCE:
            ...
            comp_result = _run_compliance(
                ...
                # DB push params — all resolved in scan.py, passed through
            ...
            if COMPLIANCE_DB_LOAD:
                ...
                if db_status == "success":
                    log(f"  DB push         : SUCCESS — {db_rows} rows → "
                        f"[{COMPLIANCE_DB_SCHEMA}].[{COMPLIANCE_DB_TABLE}]", "OK")
