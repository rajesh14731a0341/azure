...
def is_in_timeframe(col, cutoff_ts):
    times = []

    if col.get("classificationAppliedTime"):
        times.append(col["classificationAppliedTime"])

    if col.get("labelAppliedTime"):
        times.append(col["labelAppliedTime"])

    if col.get("tagsAppliedTime"):
        times.append(col["tagsAppliedTime"])

    if col.get("lastModifiedTS"):
        times.append(col["lastModifiedTS"])
...
def ms_to_iso(ms):
    ...
    try:
        ...
        return datetime.datetime.fromtimestamp(v / 1000, datetime.UTC).strftime("%Y-%m-%d %H:%M:%S")
